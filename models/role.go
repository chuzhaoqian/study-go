...
type Role struct {
	...
	Name       string    `orm:"column(name);size(20)" description:"角色名称"`
	Pid        int64     `orm:"column(pid);null" description:"父角色ID"`
	Type       int8      `orm:"column(type)" description:"类型 0 普通角色 1超级角色 2特殊"`
	Award      int8      `orm:"column(award)" description:"0 不可授予 可授予 1"`
	Status     uint8     `orm:"column(status);null" description:"状态: 0=有效 1=禁用"`
	Remark     string    `orm:"column(remark);size(255);null" description:"备注"`
	Sort       int64     `orm:"column(sort)" description:"排序字段"`
